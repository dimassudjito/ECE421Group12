...
async fn postGameAsync(
    ...
    match client.post("http://localhost:8000/game")
        .body(bod)
        .send()
        .await
    {
...
pub fn postGame(
    ...
    wasm_bindgen_futures::spawn_local(postGameAsync(game_type,
        player_1_name,
        player_2_name,
        winner_name,
        get_time(),
...
async fn updateScores(
    ...
    match reqwest::get("http://localhost:8000/computer/wins").await{
        Ok(r)=>{
            match r.json::<Vec<Game>>().await{
                Ok(body)=>{
                    let wins: Vec<Vec<String>> = body.into_iter().enumerate().map(|(i,game)| 
                        vec![
                            game.game_number.to_string(),
                            game.game_type,
                            game.winner_name,
